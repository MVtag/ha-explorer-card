...
export type RoomReactionKind = string;
...
export interface ExplorerMovementHistoryConfig { enabled?:boolean; duration_minutes?:number; show_rooms?:boolean; }
export interface ExplorerPetRobotTrailsConfig { enabled?:boolean; duration_minutes?:number; show_pet_paws?:boolean; show_robot_route?:boolean; robot_direction_arrows?:boolean; }
...
export interface ShellyPetDetectionConfig { enabled?:boolean; height_attribute?:string; target_id_attribute?:string; timestamp_attribute?:string; max_height_m?:number; release_height_m?:number; confirmation_updates?:number; release_updates?:number; }
export interface ExplorerPresence { id:string; name?:string; type?:PresenceObjectType; x?:number; y?:number; room_id?:string; color?:string; icon?:string; avatar?:string; visible?:boolean; trail_visible?:boolean; trail_color?:string; trail_duration?:number; entity_binding?:PresenceEntityBinding; shelly_pet_detection?:ShellyPetDetectionConfig; }
export interface ExplorerCardConfig { type:string; title?:string; image?:string; background?:string; floorplan_meters?:ExplorerFloorplanMeters; min_zoom?:number; max_zoom?:number; initial_zoom?:number; fit_mode?:FloorplanFitMode; appearance?:ExplorerAppearanceConfig; movement_history?:ExplorerMovementHistoryConfig; pet_robot_trails?:ExplorerPetRobotTrailsConfig; rooms?:ExplorerRoom[]; zones?:ExplorerZone[]; route_nodes?:ExplorerRouteNode[]; route_graph_edges?:ExplorerRouteGraphEdge[]; routes?:ExplorerRoute[]; openings?:ExplorerOpening[]; presences?:ExplorerPresence[]; }
