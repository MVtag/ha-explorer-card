...
function entityCoordinates(presence:ExplorerPresence,binding:PresenceEntityBinding,entity:HassEntity|undefined,floorplan?:ExplorerFloorplanMeters):{x?:number;y?:number} {
  if(!entity)return {x:normalizedNumber(presence.x),y:normalizedNumber(presence.y)};
  const xAttr=binding.x_attribute??(binding.coordinate_space==="meters"?"map_x":"explorer_x");
  const yAttr=binding.y_attribute??(binding.coordinate_space==="meters"?"map_y":"explorer_y");
  if(binding.coordinate_space==="meters"){
    ...
    if(mx===undefined||my===undefined||!width||!height||width<=0||height<=0)return {x:undefined,y:undefined};
...
export function resolvePresence(presence:ExplorerPresence,hass?:HomeAssistant,rooms:ExplorerRoom[]=[],floorplan?:ExplorerFloorplanMeters):ExplorerPresence {
  ...
  const coords=entityCoordinates(presence,binding,entity,floorplan);
  const resolved:ExplorerPresence={...presence,x:coords.x,y:coords.y,name:presence.name??stringValue(readAttribute(entity,binding.name_attribute??"friendly_name")),avatar:presence.avatar??stringValue(readAttribute(entity,binding.avatar_attribute??"entity_picture")),icon:presence.icon??(binding.icon_attribute?stringValue(readAttribute(entity,binding.icon_attribute)):undefined),color:presence.color??stringValue(readAttribute(entity,binding.color_attribute??"explorer_color")),visible};
  const roomReference=readRoomReference(binding,entity,hass)??presence.room_id;return applyRoomPosition(resolved,rooms,roomReference);
}
