...
export class HaExplorerHaEditor extends HaExplorerCardEditor {
  ...
  private readonly handleNativeControlChange=(event:Event):void=>{event.stopPropagation();queueMicrotask(()=>{const config=this.currentConfig;if(config)this.emitHomeAssistantConfig(config);});};
  ...
  protected override firstUpdated():void{this.renderRoot.addEventListener("change",this.handleNativeControlChange);this.renderRoot.addEventListener("click",this.handleItemCardClick);queueMicrotask(()=>this.baseSections.forEach(section=>section.open=false));}
  public override disconnectedCallback():void{this.renderRoot.removeEventListener("change",this.handleNativeControlChange);this.renderRoot.removeEventListener("click",this.handleItemCardClick);super.disconnectedCallback();}
  ...
  private handleEditorNavigate(event:EditorNavigateEvent):void{const target=event.detail?.target;if(!target)return;event.stopPropagation();const baseIndex=BASE_SECTION_INDEX[target];let element:HTMLElement|undefined;if(baseIndex!==undefined){const section=this.baseSections[baseIndex];if(section){section.open=true;element=section;}}else{const section=this.renderRoot.querySelector<HTMLDetailsElement>(`details[data-editor-section="${target}"]`);if(section){section.open=true;element=section;}}if(!element)return;element.classList.add("ux-focus");element.scrollIntoView({behavior:"smooth",block:"start"});window.setTimeout(()=>element?.classList.remove("ux-focus"),1300);}
  private renderAdvancedSection(target:ExplorerEditorSection,title:string,hint:string,content:unknown){return html`<details class="advanced-section" data-editor-section=${target}><summary><span>${title}</span><span class="advanced-hint">${hint}</span></summary><div class="advanced-content">${content}</div></details>`;}

  protected override render(){const config=this.currentConfig;return html`
    <details class="setup-section"><summary><span>Opsætningsoversigt</span><span class="advanced-hint">Status & genveje</span></summary><div class="setup-content"><ha-explorer-setup-overview .hass=${this.hass} .config=${config} @explorer-editor-navigate=${this.handleEditorNavigate}></ha-explorer-setup-overview></div></details>
    ${super.render()}
    ...
    <div class="advanced-tools">
      ${this.renderAdvancedSection("appearance","Appearance","Classic / Enchanted Antique",html`<ha-explorer-theme-editor .config=${config} @config-changed=${this.handleToolConfigChanged}></ha-explorer-theme-editor>`)}
      ...
      ${this.renderAdvancedSection("diagnostics","Route Preview & Diagnostics","Kontrollér graph og resolved routes",html`<ha-explorer-route-diagnostics .hass=${this.hass} .config=${config}></ha-explorer-route-diagnostics>`)}
    </div>`;}
