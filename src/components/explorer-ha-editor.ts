...
export class HaExplorerHaEditor extends HaExplorerCardEditor {
  ...
  private handleEditorNavigate(event:EditorNavigateEvent):void{const target=event.detail?.target;if(!target)return;event.stopPropagation();const baseIndex=BASE_SECTION_INDEX[target];let element:HTMLElement|undefined;if(baseIndex!==undefined){const section=this.baseSections[baseIndex];if(section){section.open=true;element=section;}}else{const section=this.renderRoot.querySelector<HTMLDetailsElement>(`details[data-editor-section="${target}"]`);if(section){const key=section.dataset.lazyEditorSection;if(key)this.setEditorSectionOpen(key,true);section.open=true;element=section;}}if(!element)return;element.classList.add("ux-focus");element.scrollIntoView({behavior:"smooth",block:"start"});window.setTimeout(()=>element?.classList.remove("ux-focus"),1300);}
